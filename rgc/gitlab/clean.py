...
class GitlabClean(object):
    def __init__(self, user, token, gitlab_url, registry_url, retention, exclude, project_filter):
        self.user = user
        self.token = token
    ...
    def clean_projects(self):
        ...
        for project in gitlab.Gitlab(self.gitlab_url, self.token).projects.list(all=True):
            if not re.match(self.project_filter, project.path_with_namespace.lower()):
                print('-> filtered ' + project.path_with_namespace.lower())
                continue
            ...
            for subimage in subimages:
                ...
                for tag in tags:
                    ...
                    created_at = datetime.strptime(
                        json.loads(tag_info['history'][0]['v1Compatibility'])['created'][:-4], '%Y-%m-%dT%H:%M:%S.%f')
                    age = now - created_at
                    if age.total_seconds() > (int(self.retention) * 60 * 60 * 24):
                        print(colored('--> removing ' + tag + ' (expired)', 'red'))
                        digest = registry.query(self.registry_url + '/v2/' + subimage + '/manifests/' + tag, 'head')[
